...
The rows corrected on 2026-09-07 were read against this repository at commit
`e941800b27ee2944bc0759cef29a403e4f98e00d`, by holding each workflow's calls and
jobs against what its row says of them. Those are the two readings a row's shape
claim rests on, so they are handed over rather than their output:

    git grep -n '^    uses:' -- .github/workflows/
    git grep -n '^  [a-z0-9-]*:$' -- .github/workflows/
...
| --- | --- | --- |
| `build.yml` (Build) | Present, and half of it is the target's shape | This row said `build.yaml` calls the shared plugin workflow instead of carrying package steps, and that stopped being true on 2026-09-01 without the row moving. The `call` job still calls the shared workflow, for the 10.11 package, and the `jf12` job beside it carries its own four steps for the 12.0 package, because the shared workflow takes a target and no manifest path and the packaging tool reads the one manifest under the sources path it is given, so a second call would package the same line twice. The trade the row named is half kept: one package is a dependency on somebody else's workflow file and the other is steps in this tree, and the header of `build.yaml` says why. The inventory is the half that is no longer a difference: the target generates one in an isolated job off the locked restore graph, and `publish.yaml` here does the same, on the release route rather than on every build, because the inventory this repository owes is the one describing what a release shipped. |
| `codeql.yml` (CodeQL) | Present, required, and analysed in this tree | This row said the check was present and not required and that `scan-codeql.yaml` calls the shared workflow, and both halves stopped being true without it moving. The analysis was brought into this tree on 2026-08-12 so the query pack and the severity threshold could be set beside the code they read, and the context it produces, `Code scanning (csharp)`, is in the required set the commands above print. This row then said #80 still owed a release carrying the inventory and the attestation, and #80 closed on 2026-09-03 with the first release carrying both, without the row moving. `gh release view 0.1.1.0-stable --json assets --jq '.assets[].name'` prints the inventory and its checksum beside the archive. |
...
| `dotnet.yml` (.NET) | Partly present, and the floor is not a separate job | This workflow carries the target's `build`, `ABI floor build` and `Package (JPRM)` jobs. Here `build.yaml` and `test.yaml` call the shared workflows for the first. This row said there was no ABI floor build because a floor needs the supported server lines fixed first, and #9 fixed them, both lines targeted, without the row moving. What the target's floor job does is build once more against the server package at the floor its manifest declares, so a member newer than the floor is refused. Here that is not a second job: the plugin project references, per target framework, the server package of the line each manifest declares, and `EveryManifestsTargetAbiAgreesWithTheServerPackagesForItsFramework` in `ManifestTests` refuses a manifest whose `targetAbi` names a version below the one its framework's build binds against. So every build here compiles at or below the floor, for both lines, at the cost `docs/supported-servers.md` records: the 12.0 line's reference is a release candidate. |
...
| `manifest-freshness.yml` | Adopted, under another name and with a wider comparison | This row said the shape was not adopted because there was no published channel to assert against. There is one: the catalogue at the address the readme gives an operator carries an entry for this plugin, and on 2026-09-04 it advertised a version older than the newest release. `channel-freshness.yml` reads the served file against this repository's releases daily and reports both directions - a version published and not advertised, and a version advertised with no release behind it - where the target asserts one direction on a beta manifest. It reports and does not repair, because the catalogue is another board's tree and nothing here writes to it, and it is not in this repository's required set. It is the third condition of #88; the rest of that issue is not this row. |
| `nightly-betas.yml` | Not adopted yet, and the condition this row named has arrived | This row waited on decision 4 in #1, and that decision is taken: 1.0 carries both server lines. What a nightly beta channel would publish is therefore known, and no issue on this board asks for one. That is a gap this document records rather than closes. |
| `opengrep.yml` (Repo Invariant Lint) | Adopted, inside the suite rather than as a workflow | This row said not adopted yet, and #79 closed on 2026-08-08 without it moving. The lint is a rule table in `InvariantLintTests`, seeded from the invariants the contributing guide declares rather than copied from the target, and it rides the required `call / test` context rather than adding one of its own, because a new context needs a ruleset edit no pull request can make. `git grep -n 'Id: "' -- Jellyfin.Plugin.MetadataSync.Tests/InvariantLintTests.cs` prints the rules. |
...
| `publish-beta.yml` | Not adopted yet, and the condition has arrived | A beta channel. This row waited on the same decision 4 in #1 as the nightly one, and it is taken. No issue on this board asks for a beta channel, which the nightly row already records. |
| `publish-jf12-beta.yml` | Not adopted yet, and the condition has arrived | A per-generation beta channel. Decision 4 in #1 fixed how many generations are published, at two, and the row did not move for it. The same gap as the two rows above. |
| `publish-jf12-stable.yml` | Not adopted yet, and the nearest thing to it is owed | A per-generation stable channel. This row waited on decision 4 in #1, which is taken. The target publishes its second generation from a workflow of its own; here the second package is built by the `jf12` job in `build.yaml` and the release route reads `build.yaml` alone, so the 12.0 package has no release leg. #377 is where one is owed, and whether it is a second workflow or a second manifest read in the one route is decided there. |
| `publish-failure-alert.yml` | Not adopted yet, and the condition has arrived | The target sweeps for any workflow that concluded non-success on its default branch. This row said it was useful only once there is a publication that can fail unattended, and there is one: two releases have been cut through `publish.yaml`, and `channel-freshness.yml` and `label-drift.yml` run on a schedule with nobody watching. Nothing here sweeps for a failed run, and no issue on this board asks for it. |
| `publish.yml` (Publish Release) | Present, and the same shape as the target's | This row said `publish.yaml` calls the shared plugin publish workflow rather than carrying its own steps, and that stopped being true on 2026-08-08, two days after the row was written, when #130 brought the pipeline into this tree. It has carried its own jobs since, and none of them calls another repository's workflow: `git show origin/master:.github/workflows/publish.yaml \| grep -c '^    uses:'` prints 0. The trade the row named is not made here at all, and the difference that matters to a reader of this table is the opposite of the one the row gave: every release refusal is in this tree and is this board's to change, and `docs/RELEASING.md` lists them. What the route publishes is one of the two packages, the 10.11 line's; the 12.0 package's release leg is #377. |
| `regenerate-manifest.yml` | Not adopted, and the condition has moved | Manual regeneration of a published manifest, which presumes a published manifest. This row held the answer on #88, which is closed: the catalogue exists, it is built on another board out of this repository's releases, and nothing in this tree writes a manifest, so there is nothing here to regenerate. The condition that changes the answer is this repository writing its own catalogue, which `docs/RELEASING.md` says it does not. |
...
| `zizmor.yml` (Workflow Security Analysis) | Present, required | This row said the audit was not required and that putting it in the required set was #77. #77 closed on 2026-08-16 with the context `Audit workflows (zizmor)` in the set, and the row did not move. Whether it is there today is what the commands above print. |
...
| `label-drift.yml` | Added by #261, when `sync-labels.yaml` sat above it and applied `.github/labels.yaml` with every other label deleted, so that file was the whole board rather than a list of labels the board should have among others. That workflow is gone (#351) and this one is what is left: the label file is now a declaration nothing applies, and a difference either way still costs something. A label the board carries and the file does not declare is no longer deleted by a run, so it stays until somebody removes it; a label the file declares and the board does not carry is offered by the hygiene gate as a repair that does not exist. This reads the two against each other on a schedule and reports both directions. It repairs neither side, and it is not in this repository's required set. The list above carries no equivalent on the target. |
...
| `headless.yml` | Added by #72. It runs the suite in a container with no display server and no network, so a test that quietly reached one fails here where the required `call / test` on a hosted runner would let it pass. `docs/testing.md` states the policy this makes a fact, and `TestingStatementTests` reads the job and the flag out of the file rather than out of the sentence. The list above carries no equivalent on the target, and it is not in this repository's required set. This row was missing for a month after the workflow landed, which is the unexplained gap the opening of this document calls a defect. |

## What this document does not say

It is read by nothing. No test in the suite holds a row of either table against
the workflow it describes, and nothing else does. That is how the publish row
described another repository's file for a month while this one carried its own
jobs, and how five more rows stood past the issue that changed their answer.
The rows are held by whoever next runs the two commands under the comparison
heading against them, which is a person and not a mechanism. Whether a reader in
the suite is owed is not decided here.
