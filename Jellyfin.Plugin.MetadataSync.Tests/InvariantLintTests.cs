    ...
    private static readonly Rule[] Rules = new[]
    {
        new Rule(
            ...
            AllowedIn: new[]
            {
                ...
                ("ConflictLog.cs",
                 "Is the plugin's third store, which keeps the account of what this plugin decided in a file beside the plugin's data, so it names the file-system types for the same reason WrittenValues does. Nothing here takes part in deciding that two items are the same: the item arrives as an identifier the resolver already produced, a decision is written down and read back, and no property of a file, a path or a directory reaches a comparison. The allowance costs what the two beside it cost - a file-system read added to this file for another reason is not refused by this rule - and what stands against that is the review and the walk in ResolutionPathTests, which starts at the resolvers and reaches no store."),
                ("PassProgress.cs",
                 "Is the plugin's second store, which keeps the items an interrupted pass had finished with in a file beside the plugin's data, so it names the file-system types for the same reason WrittenValues does. It holds less than that store rather than more: a line is a pairing and an item identifier the resolver already produced, no value and no name out of a library reaches it, and nothing here compares anything at all. The allowance costs what the one above it costs - a file-system read added to this file for another reason is not refused by this rule - and what stands against that is the review and the same walk in ResolutionPathTests."),
            },
